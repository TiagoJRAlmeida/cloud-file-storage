...
Lastly, the Cloud version, folder `/GCP`. This one is the important one. However I haven't touched it since a long time ago so
there are a lot of things (although now it's easy for me to fix) to do. Here are some out of the top of my head:

- The Terraform stuff:
    - I am not sure about the storage part. I think it should be used to save the terraform state, but I am not sure, and currently is
    configured to save the data form MinIO server (Which now I now it doesn't make sense, the Kubernetes cluster volumes deal with that).
